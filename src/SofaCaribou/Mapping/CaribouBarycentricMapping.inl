...
: d_topology(initLink("topology", "Topology that contains the embedded (mapped) nodes."))
{
}
...
void CaribouBarycentricMapping<Element, MappedDataTypes>::draw(const sofa::core::visual::VisualParams *vparams) {
    ...
    if (not p_barycentric_container->outside_nodes().empty()) {
        ...
        std::vector<sofa::defaulttype::Vector3> outside_nodes_positions (outside_nodes.size());
